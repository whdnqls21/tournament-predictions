...
- ✅ **대진표** — 32강→결승 **좌우 스크롤 브라켓**(슬롯 0,1 승자 → 다음 라운드), 내 예측 겹쳐보기(적중 초록/빗나감 빨강·진출 강조)
...
- ✅ **미니게임(스코어 맞히기)** — 메인 게임과 **별개**. 관리자가 브라켓 경기를 **여러 개** 추가하면, 참가자가 **정확 스코어**를 맞힌다(±로 입력 후 **제출**, 제출 회수 가능, 시작 시간에 자동 마감→공개, 관리자가 실제 스코어 입력 시 적중자 표시). 하단 **🎯 미니게임** 탭, 시작 시간 빠른순 정렬.
...
> **이미 배포된 DB 업데이트(미니게임)**: 마찬가지로 기존 DB라면
> [`supabase/migrations/0002_minigame.sql`](supabase/migrations/0002_minigame.sql)
> 을 SQL Editor 에 붙여넣고 **Run** 한다(settings 에 미니게임 컬럼 + `mini_predictions` 테이블 추가).
> 이어서 **여러 개 동시 운영**을 위해
> [`supabase/migrations/0003_mini_games_multi.sql`](supabase/migrations/0003_mini_games_multi.sql)
> 도 실행한다(`mini_games` 테이블 추가 + 기존 1개 자동 이관).
...
1. 관리자 진입은 **숨김 제스처**다 — 상단 헤더의 **🏆 로고를 빠르게 5번 탭**하면 관리자 화면(`/admin`)으로 이동한다. (설치형 PWA 라 주소창이 없어도 진입 가능) → PIN 입력. **처음 입력한 PIN 이 관리자 PIN 으로 등록**된다. (분실 시 Supabase `settings.admin_pin_hash` 를 비우면 재설정)
...
1. 공유받은 URL 접속 → 이름 선택 + 4자리 PIN 으로 로그인.
   - 로그인 후 예측 탭 우상단 **PIN 변경** 으로 본인 PIN 을 직접 바꿀 수 있다(현재 PIN 확인 후 새 PIN 설정).
...
접속 시 상단에 **설치 안내 배너**가 뜬다(닫으면 다시 안 뜸).

- **Android(Chrome)**: 배너의 **"추가"** 버튼 한 번이면 설치(원탭). 설치 조건을 위해 PNG 아이콘(192·512)과 최소 서비스워커(`/sw.js`)를 제공한다.
- **iPhone(Safari)**: iOS 는 정책상 **자동 설치 팝업이 없다.** 배너 안내대로 **공유 버튼 → "홈 화면에 추가"** 를 직접 눌러야 한다.

아이콘·이름("축잘알")·상태바 색까지 다크 그린 테마로 설정돼 있다. (웹 매니페스트 + PNG/SVG 아이콘 + apple-touch-icon 적용)
