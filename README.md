...
- ✅ **관리자 셋업** — 32강 16경기 대진 배치, 참가자 PIN 지정/재설정
- ✅ **예측·마감** — 라운드별 승팀 선택·확정·확정취소, 전원 확정 시 자동 마감 + 관리자 강제 마감, 마감 전 가림(누가 확정했는지만 공개)
- ✅ **대진표** — 전체 브라켓, 내 예측 겹쳐보기(적중 초록/빗나감 빨강), 마감 라운드는 4명 예측 공개
...
2. **32강 셋업**: 16경기의 두 팀을 입력하고, 원하면 참가자 4명 PIN 도 함께 지정 → "대회 시작!".
3. 이후 라운드마다:
   - 참가자들이 예측·확정 → 4명 전원 확정 시 자동 마감(또는 **강제 마감** 버튼).
   - 마감되면 각 경기에서 **진출(승리)한 팀을 눌러 결과 입력**.
...
2. **예측 탭**: 열린 라운드의 각 경기에서 이길 팀을 고르고 **확정**. 마감 전이면 "확정 취소" 후 수정 가능.
3. **대진표 탭**: 전체 브라켓 + 내 예측 적중/빗나감. 마감된 라운드는 4명 예측이 모두 공개된다.
